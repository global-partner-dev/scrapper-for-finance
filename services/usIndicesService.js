...
/**
 * Inserts US indices data into Supabase
 * @param {Array} indices - Array of index objects from scraper
 * @returns {Promise<Object>} Result with success status and details
 */
async function saveUSIndices(indices) {
  try {
    ...
    console.error('Exception in saveUSIndices:', error);
    return {
      success: false,
      message: error.message,
      saved: 0,
...
async function getIndicesByDateRange(startDate, endDate) {
  try {
    const { data, error } = await supabase
      .rpc('get_us_indices_by_date_range', {
...
async function cleanupOldData(daysToKeep = 30) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .rpc('cleanup_old_us_indices_data', {
